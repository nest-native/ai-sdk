...
- Supply-chain checks are NON-NEGOTIABLE:
  ...
  - `packages/ai/package.json` must keep `"dependencies": {}`.
...
- Version drift between `packages/ai` and `sample/*` is a release blocker.
- AI SDK version pin is critical: v5 is a hard requirement; document
  non-support of older majors prominently.
- When bumping `packages/ai/package.json`, update all
  `sample/*/package.json` entries for `"nest-ai-native"` in the same change.
...
- When changes touch `packages/ai/**/*.ts`, run `npm run complexity:check`
