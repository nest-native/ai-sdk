...
// compatibility leg in CI runs this after its `--no-save` install: npm nests an
// older copy under a workspace whenever that workspace's own range is not
// satisfied by the hoisted version, and a suite (or a sample smoke) that passes
// against a mixed 11/12 tree proves nothing about 12.
//
// Every workspace is checked for `@nestjs/common` and `@nestjs/core`, plus every
// other `@nestjs/*` package that workspace declares itself (the platform
// adapters in the samples, `@nestjs/testing` at the root).
...
for (const workspaceDir of ['.', ...collectWorkspaceDirs()]) {
  ...
  for (const packageName of collectNestPackages(manifestPath)) {
    const { version, packageDir } = resolveInstalled(require, packageName);
    const major = Number(version.split('.')[0]);

    console.log(
      `${workspaceDir.padEnd(26)} ${packageName.padEnd(26)} ${version.padEnd(8)} <- ${path.relative(repoRoot, packageDir)}`,
    );
...
function collectNestPackages(manifestPath) {
