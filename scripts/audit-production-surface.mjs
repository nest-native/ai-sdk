/**
 * Audits the *published* supply-chain surface of `@nest-native/ai-sdk`.
 *
 ...
 * peer, or `@ai-sdk/provider-utils` via the sample apps' `ai` dependency) — none
 * of which can reach a consumer. `npm audit --omit=dev` cannot prune those at the
 * root because npm audits the whole shared-lockfile ideal tree regardless of
 * `--omit`.
 *
...
const npmCache = fs.mkdtempSync(path.join(os.tmpdir(), 'nest-native-ai-sdk-audit-cache-'));
const consumerDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nest-native-ai-sdk-audit-consumer-'));
...
try {
  ...
  npm(['run', 'build', '--workspace', '@nest-native/ai-sdk'], { cwd: repoRoot, stdio: 'inherit' });

  const packOutput = npm(['pack', '--json', '--workspace', '@nest-native/ai-sdk'], {
    cwd: repoRoot,
  });
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: 'nest-native-ai-sdk-audit-consumer', version: '0.0.0', private: true }, null, 2)}\n`,
  );
